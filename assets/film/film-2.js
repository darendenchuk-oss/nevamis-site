...
var copyEls = [
  ...
  /* the ending copy waits for the finished mark (owner note 2026-09-14): it used to
     arrive at 0.90, while panes 05 and 06 were still coming into view and before
     the arch had been drawn. The stroke parks at ~0.978. */
  { el: document.getElementById('close'), p0: 0.975, p1: 1.01, at: 0.99 }
];
...
var spineMat = new T.ShaderMaterial({
  ...
  fragmentShader: [
    ...
    'uniform float uLen; uniform float uTip; uniform float uPark; uniform float uGone; uniform vec3 uOrbC; uniform float uOrbR;',
    ...
    /* finale (owner note 2026-09-14): once the mark is drawn, the rest of the line
       fades back along its length, from the tail toward the arch, and finishes
       behind pane 05 GROW. GROW hides the spine from about 0.67 to 0.73 of its
       length in the parked frame (measured at 1440x900 and 390x844), so the soft
       edge settles inside that stretch and only the arch and the dot remain.
       uGone is 0 before the finale, where keep is 1 along the whole line. */
    ' float cutE = mix(-0.2, uArchT - 0.02, uGone);',
    ' float keep = smoothstep(cutE - mix(0.18, 0.065, uGone), cutE, vT);',
    '#ifdef NECK',
    ...
    ' gl_FragColor = vec4(outc * vWake * keep, smoothstep(0.0, 0.6, m) * vWake * keep);',
    '#else',
    ' gl_FragColor = vec4(outc * vWake * keep, 1.0);',
    '#endif',
    '}'
  ].join('\n'),
  blending: T.AdditiveBlending, transparent: true, depthWrite: false
...
/* Sample count is chosen ONCE, from the display's pixel density, and never
   changes afterwards: reallocating a multisampled framebuffer mid-scroll is
   exactly the kind of hitch the governor is not allowed to cause. A hi-DPI panel
   already resolves thin lines across more pixels, so 2 samples there recovers
   almost all of the coverage (measured 0.656 vs 0.679) at half the multisample
   bandwidth - which on integrated graphics is what was pushing the governor into
   stepping down at DPR 1.5. */
...
function apply(p){
  ...
  spineMat.uniforms.uGone.value = smooth(0.93, 0.985, p); /* the corridor fades back behind GROW as the arch completes */
