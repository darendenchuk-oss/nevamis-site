...
| F15 (was O10, partial) | Sitemap lastmod maintained by hand | LOW | scripts/gen-sitemap.mjs keeps each page's recorded lastmod (config/sitemap-hashes.json) until the page's content changes, then takes the change's commit date; check-generator-drift.mjs fails when a page changed without a regenerated sitemap (2026-09-25). Vertical landing pages (WEB-061) still wait for sales evidence. |
