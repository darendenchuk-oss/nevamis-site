...
<style>
  /* ---------- page layout only ----------
     ...
     left here is the geometry this template alone has, plus four corrections a
     generated page needs that the shared sheet does not yet make. Nothing
  ...
  /* THE EYEBROW INSIDE A SECTION HEAD. p.lede,.section-head p is (0,1,1) and
     .eyebrow is (0,1,0), so every eyebrow written as a <p> inside a
     .section-head lost its family, its size and its colour and rendered as a
     17px line of body text in lede grey carrying the label's .3em of tracking.
     That is the same specificity defect the card rule already guards against
     with :not(.eyebrow), and the shared fix is the same guard on
     .section-head p. Reported, not made here. */
  .section-head p.eyebrow{font-family:var(--f-mono);font-size:var(--fs-label);
    line-height:1.3;color:var(--emerald);max-width:none}
  ...
  /* The numbered path. list-style:none was an inline attribute on the <ol>,
     ...
     and the native marker would sit outside a list whose padding * already
     zeroed. */
  ...
  /* The tick and the cross are generated content whose value ends in a space,
     and that space collapses away at the start of the cell's line box, so on
     both comparison pages every verdict rendered as one welded word. A margin
     cannot collapse. Shared rule, reported. */
  ...
  /* At 360 the table's own min-content measured 331px inside a 318px wrapper,
     ...
     type below 380 takes the min-content to about 307 and it fits. Measured on
     both comparison pages, not derived. Shared rule, reported. */
