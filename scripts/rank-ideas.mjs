...
/* ---------- who can actually do this ----------
   Two signals, both read off the idea itself rather than guessed:

   1. Does `touches:` name a real file? An idea that edits home.html or
      src/lib/mailer.ts is something an agent can implement. One that only
      touches "process", "calendar" or PLAYBOOK.md is a habit or a decision.

   2. Does the verb require a person in the world? Booking a lawyer, opening
      a bank account, phoning a competitor, or choosing what the referral fee
      is are not code, however clearly they are written.

   Ambiguity resolves toward BUILDABLE on purpose: the point of this list is
   to be handed instructions, and a human vetoing one row costs far less than
   a good idea never being seen. */
const FILE_RE = /\b[\w./-]+\.(html|js|mjs|ts|tsx|css|json|xml|txt|csv)\b|\bsrc\/|\bscripts\/|\bassets\//;

const OWNER_ONLY = [
  { why: 'professional advice', re: /\b(lawyer|solicitor|legal review|reviewed by an? (alberta )?lawyer|accountant|insurance|incorporat|trademark|NUANS)\b/i },
  { why: 'money or accounts', re: /\b(bank account|open an? account|buy (a|an|the)|purchase|pay for|deposit|fund the)\b/i },
  { why: 'a conversation', re: /\b(call a competitor|phone a|visit|meet |in-person|interview|hire|recruit|a peer|ask (a friend|someone)|talk to)\b/i },
  { why: 'the founder\'s own time', re: /\b(block (calling )?hours|day off|energy log|shutdown ritual|calendar as an appointment|take one full day)\b/i },
  { why: 'a decision only you can make', re: /^(decide|choose|pick one|set the price|name the|write down what would make you stop)/i },
];

function classify(i, body) {
  const owner = OWNER_ONLY.find((o) => o.re.test(i.title) || o.re.test(body));
  if (owner) return { who: 'you', why: owner.why };
  if (FILE_RE.test(body)) return { who: 'agent', why: '' };
  return { who: 'you', why: 'a habit or a document, not code' };
}
...
> **Prices quoted inside idea titles below are RETIRED.** The backlog was authored against the
> C\$249 / C\$449 / C\$849 ladder, Pay As You Go, annual prepay, a setup fee and a 7-day pilot.
> Since 2026-08-09 the model is one recurring price per plan: Core C\$250/month, Growth
> C\$500/month, Pro C\$1,000/month, nothing charged beside it, no pilot and no trial. This file is
> generated from the idea documents, so their old figures come through verbatim; \`pricing-config.js\`
> is the source of truth and the per-domain files carry the same warning at the top.
...
/* ---------- the buildable list ---------- */
for (const i of ideas) Object.assign(i, classify(i, i.body));
const buildable = now.filter((i) => i.who === 'agent');
const yours = now.filter((i) => i.who === 'you');

const byDomain = buildable.reduce((m, i) => ((m[i.domain] ??= []).push(i), m), {});
const domainOrder = Object.keys(byDomain).sort((a, b) => (STAGE_FIT[b] ?? 0.4) - (STAGE_FIT[a] ?? 0.4));

fs.writeFileSync(path.join(dir, 'IMPLEMENTABLE.md'), `# What can be built without you
...
**${buildable.length} of ${ideas.length} ideas are code or content changes** an agent can
implement directly. ${yours.length} need you (a decision, a conversation, a lawyer, a bank),
${later.length} are blocked on clients or traffic, and completed ones are excluded via DONE.md.
...
${domainOrder.map((d) => {
  ...
  return `### ${d} — ${list.length} buildable\n\n`
    + '| Score | ID | Improvement | Impact | Effort |\n|-------|----|-------------|--------|--------|\n'
    + list.map((i) => `| ${i.score.toFixed(2)} | \`${i.id}\` | ${i.title} | ${i.impact} | ${i.effort} |`).join('\n');
}).join('\n\n')}
...
|-------|----|-------------|------------------|
${yours.slice(0, 120).map((i) => `| ${i.score.toFixed(2)} | \`${i.id}\` | ${i.title} | ${i.why} |`).join('\n')}
${yours.length > 120 ? `\n_${yours.length - 120} more in RANKED.md._` : ''}
`);

console.log(`RANKED.md written: ${ideas.length} ideas (${now.length} now, ${later.length} blocked)`);
console.log(`IMPLEMENTABLE.md written: ${buildable.length} buildable, ${yours.length} need you`);
console.log(`Top 5:`);
